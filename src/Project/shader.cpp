...
#include <iostream>
	...
	namespace Rendering
	{
		...
		bool Shader::CreateShader(const std::string & shaderSourceV, const std::string& shaderSourceF)
		{
			if (!m_successful)
			{
				...
				const GLchar *source = shaderSourceV.c_str();
				GLint length = shaderSourceV.size();

				glShaderSource(m_vertexShader, 1, &source, &length);
				glCompileShader(m_vertexShader);
				if (HasCausedError(m_vertexShader, GL_COMPILE_STATUS, shaderSourceV))
				{
				...
				source = shaderSourceF.c_str();
				length = shaderSourceF.size();

				glShaderSource(m_fragmentShader, 1, &source, &length);
				glCompileShader(m_fragmentShader);
				if (HasCausedError(m_fragmentShader, GL_COMPILE_STATUS, shaderSourceF))
				{
					DestroyShaderObjects();
					return false;
				}
		...
		std::string LoadShader(const std::string &file)
		{
			std::string file_location = file;// Util::GetExecutablePath() + "/game_data/shader/" + file;
